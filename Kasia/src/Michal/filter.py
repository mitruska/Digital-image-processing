    ...
    def reliefGray(self, show = False):
        ...
        for i in range(height):
            for j in range(width):
                ...
                #resultImage[i, j] = min(map(abs, value))
                #resultImage[i, j] = sum(map(abs, value)) / len(value)
                resultImage[i, j] = max(map(abs, value))

        if show:
            self.show(Image.fromarray(resultImage, "L"))
        self.save(resultImage, self.imName, "reliefSobol")
    ...
    def reliefColor(self, show = False):
        ...
        for i in range(height):
            for j in range(width):
                ...
                #resultImage[i, j] = (min(map(abs, r)), min(map(abs, g)), min(map(abs, b)))
                #resultImage[i, j] = (sum(map(abs, r)) / len(r), sum(map(abs, g)) / len(g), sum(map(abs, b)) / len(b))
                resultImage[i, j] = (max(map(abs, r)), max(map(abs, g)), max(map(abs, b)))
